...
public:
  ...
  explicit ranked_buffer_insertion( Ntk const& ntk, mockturtle::node_map<uint32_t, Ntk> const& levels, std::vector<uint32_t> const& po_levels, ranked_buffer_insertion_params const& ps = {} ) :
          _ntk(ntk),
          _ps(ps),
          _levels(levels),
...
public:
  ...
  void schedule()
  {
    ...
    if ( _ps.scheduling == ranked_buffer_insertion_params::better_depth || _ps.scheduling == ranked_buffer_insertion_params::ASAP_depth || _ps.scheduling == ranked_buffer_insertion_params::ALAP_depth )
    {
...
private:
  ...
  void compute_mobility_ASAP( mockturtle::fanout_view<Ntk> const& ntk, node const& n, mockturtle::node_map<uint32_t, Ntk>& mobility, bool try_regular )
  {
...
void remove_buffer_and_splitter_chains( BufNtk& ntk ) noexcept
{
  ...
  ntk.foreach_po( [&]( signal const& po, auto index ) {
    const auto po2 = chase( po );
    if ( po2 != po )
    {
      // You need an API to set output at index; if not available, do manual storage update.
      // For your `technology_network`, you already do `_storage->outputs[index] = ...` in `substitute_po_signals`.
