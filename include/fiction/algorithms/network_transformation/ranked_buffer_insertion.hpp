//
// Created by benjamin on 2/5/26.
//

#ifndef FICTION_RANKED_BUFFER_INSERTION_HPP
#define FICTION_RANKED_BUFFER_INSERTION_HPP
 ...
 */
struct ranked_buffer_insertion_params
{
    /*! \brief Technology assumptions. */
    mockturtle::aqfp_assumptions_realistic assume;
...
class ranked_buffer_insertion
{
  public:
    using node   = typename Ntk::node;
    using signal = typename Ntk::signal;

    explicit ranked_buffer_insertion(Ntk const& ntk, ranked_buffer_insertion_params const& ps = {}) :
            _ntk(ntk),
            _ps(ps),
            _levels(_ntk),
            _po_levels(_ntk.num_pos(), 0u),
            _timeframes(_ntk),
            _start_id(0),
            _fanouts(_ntk),
            _num_buffers(_ntk)
    {
        ...
        assert(_ps.scheduling != ranked_buffer_insertion_params::provided);
    ...
    explicit ranked_buffer_insertion(Ntk const& ntk, mockturtle::node_map<uint32_t, Ntk> const& levels,
                                     std::vector<uint32_t> const&          po_levels,
                                     ranked_buffer_insertion_params const& ps = {}) :
            _ntk(ntk),
            _ps(ps),
            _levels(levels),
            _po_levels(po_levels),
            _timeframes(_ntk),
            _start_id(0),
            _fanouts(_ntk),
            _num_buffers(_ntk)
...
#pragma region Query
    mockturtle::node_map<uint32_t, Ntk> const& levels() const
    {
        return _levels;
    }
    ...
    std::vector<uint32_t> pi_levels() const
    {
        ...
        _ntk.foreach_pi([&](auto n) { lvls.emplace_back(_levels[n]); });
        return lvls;
    }
...
#pragma region Count buffers
  ...
  private:
    ...
    uint32_t num_splitter_levels(node const& n) const
    {
        ...
        if (_ntk.is_pi(n))
        {
            if (_ntk.fanout_size(n) > _ps.assume.ci_capacity)
                return std::ceil(std::log(_ntk.fanout_size(n) - _ps.assume.ci_capacity + 1) /
                                 std::log(_ps.assume.splitter_capacity));
            else
                return 0u;
        }
    ...
    void update_fanout_info()
    {
        ...
        _ntk.foreach_po([&](auto const& f, auto i) { insert_extref(_ntk.get_node(f), i); });
    ...
    bool update_fanout_info(node const& n)
    {
        ...
        for (auto it = _fanouts[n].begin(); it != _fanouts[n].end(); ++it)
        {
            if (it->fanouts.size())
            {
                for (auto it2 = it->fanouts.begin(); it2 != it->fanouts.end(); ++it2) fos.push_back(*it2);
            }
            if (it->extrefs.size())
            {
                for (auto it2 = it->extrefs.begin(); it2 != it->extrefs.end(); ++it2) extrefs.push_back(*it2);
            }
        }

        _fanouts[n].clear();
        for (auto& fo : fos) insert_fanout(n, fo);
        for (auto& po : extrefs) insert_extref(n, po);
    ...
    bool count_edges(node const& n)
    {
        ...
        if constexpr (verify)
        {
            return _ntk.is_pi(n) ? fo_infos.front().num_edges <= _ps.assume.ci_capacity :
                                   fo_infos.front().num_edges == 1u;
        }
        else
        {
            assert(_ntk.is_pi(n) ? fo_infos.front().num_edges <= _ps.assume.ci_capacity :
                                   fo_infos.front().num_edges == 1u);
            return true;
        }
    }
...
#pragma region Initial level assignment
  public:
    void set_scheduling_policy(ranked_buffer_insertion_params::scheduling_policy p)
    {
        _ps.scheduling = p;
    }
    ...
    void schedule()
    {
        if (_ps.scheduling == ranked_buffer_insertion_params::provided)
        {
            _ntk.foreach_po(
                [&](auto const& f, auto i)
                {
        ...
        if (_ps.scheduling == ranked_buffer_insertion_params::better_depth ||
            _ps.scheduling == ranked_buffer_insertion_params::ASAP_depth ||
            _ps.scheduling == ranked_buffer_insertion_params::ALAP_depth)
        {
            mockturtle::fanout_view<Ntk> f_ntk{_ntk};
            /* Optimum-depth ALAP scheduling */
            ALAP_depth(f_ntk);
            count_buffers();
            auto const num_buf_ALAP_depth = num_buffers();

            if (_ps.scheduling == ranked_buffer_insertion_params::ALAP_depth)
                return;
            ...
            if (_ps.scheduling == ranked_buffer_insertion_params::ASAP_depth)
                return;
        ...
        ASAP();
        if (_ps.scheduling == ranked_buffer_insertion_params::ALAP)
        {
            ALAP();
        }
        else if (_ps.scheduling == ranked_buffer_insertion_params::better)
        {
            count_buffers();
     ...
     */
    void ASAP_depth(mockturtle::fanout_view<Ntk> const& f_ntk, bool try_regular)
    {
        mockturtle::node_map<uint32_t, Ntk> mobility(_ntk, std::numeric_limits<uint32_t>::max());
        ...
        _ntk.foreach_gate([&](auto const& n) { compute_mobility_ASAP(f_ntk, n, mobility, try_regular); });
    ...
    void ALAP_depth(mockturtle::fanout_view<Ntk> const& f_ntk)
    {
        _levels.reset(0);
        mockturtle::topo_view<Ntk> topo_ntk{_ntk};

        /* compute ALAP */
        _depth             = std::numeric_limits<uint32_t>::max() - 1;
        uint32_t min_level = std::numeric_limits<uint32_t>::max() - 1;
        topo_ntk.foreach_node_reverse(
            [&](auto const& n)
            {
        ...
        _ntk.foreach_gate([&](auto const& n) { _levels[n] = _levels[n] - delta; });
        _depth -= delta;
        assert(_depth % _ps.assume.num_phases == 0);
        if (_ps.assume.balance_cios)
        {
  ...
  private:
    ...
    void compute_levels_ALAP_depth(mockturtle::fanout_view<Ntk> const& ntk, node const& n)
    {
        std::vector<uint32_t> level_assignment;
        level_assignment.reserve(ntk.fanout_size(n));

        /* if node is a PO, add levels */
        for (auto i = ntk.fanout(n).size(); i < ntk.fanout_size(n); ++i) level_assignment.push_back(_depth + 1);

        /* get fanout levels */
        ntk.foreach_fanout(n, [&](auto const& f) { level_assignment.push_back(_levels[f]); });
    ...
    void compute_mobility_ASAP(mockturtle::fanout_view<Ntk> const& ntk, node const& n,
                               mockturtle::node_map<uint32_t, Ntk>& mobility, bool try_regular)
    {
        ...
        /* get fanout levels */
        ntk.foreach_fanout(n,
                           [&](auto const& f) { level_assignment.push_back({ntk.node_to_index(f), _levels[f], 0}); });
        ...
        /* sort by descending order of levels */
        std::stable_sort(level_assignment.begin(), level_assignment.end(),
        ...
        for (auto i = 0; i < level_assignment.size(); ++i)
        {
            ...
            else
            {
                ...
                if (mobility_update)
                {
                    for (auto j = 0; j < i; ++j) level_assignment[j][2] += mobility_update;
                }
...
#pragma region Dump buffered network
  public:
    template <typename BufNtk>
    bool tie_break_by_fanout_walk(const BufNtk& bufntk, node a, node b, node root) const
    {
        mockturtle::fanout_view fanout_ntk(bufntk);

        std::vector<node> frontier_a{a};
        std::vector<node> frontier_b{b};

        // Safety to avoid infinite loops (should not happen in a DAG)
        const auto max_iters = static_cast<std::size_t>(fanout_ntk.size()) + 1u;

        const auto start_level = bufntk.level(a);

        auto backtrack_to_start_level = [&](node start, uint32_t steps, uint32_t c_a) -> node
        {
            const auto target_level = start_level - 1u;

            if (bufntk.level(start) <= target_level)
            {
                return start;
            }
            ...
            stack.reserve(64);
            stack.emplace_back(start, steps);

            while (!stack.empty())
            {
                const auto [cur, rem] = stack.back();
                stack.pop_back();

                const auto lvl = bufntk.level(cur);
                ...
                // Push eligible predecessors directly (LIFO => DFS).
                // Since foreach_fanin can't early-exit, we just push all candidates.
                bufntk.foreach_fanin(cur, [&](const auto& fi)
                {
                    const auto p = bufntk.get_node(fi);

                    // Prune: don't even explore nodes already colored by A
                    if (fanout_ntk.visited(p) == c_a)
                    {
                        return;
                    }

                    fanout_ntk.set_visited(p, c_a);
                    stack.emplace_back(p, rem - 1u);
                });
            }
        ...
        const auto color_a = fanout_ntk.trav_id();
        fanout_ntk.incr_trav_id();
        const auto color_b = fanout_ntk.trav_id();
        fanout_ntk.incr_trav_id();
        const auto color_a_and_b = fanout_ntk.trav_id();

        fanout_ntk.set_visited(a, color_a);
        fanout_ntk.set_visited(b, color_b);

        for (std::size_t iter = 0u; iter < max_iters; ++iter)
        {
            // Mark fanouts of A-frontier
            for (const auto& x : frontier_a)
            {
                fanout_ntk.foreach_fanout(x, [&fanout_ntk, &color_a](const auto& f)
                {
                    fanout_ntk.set_visited(f, color_a);
                });
            }

            // Mark fanouts of B-frontier, detect overlaps -> color_a_and_b
            for (const auto& x : frontier_b)
            {
                fanout_ntk.foreach_fanout(x,
                                          [&fanout_ntk, &color_b, &color_a, &color_a_and_b](const auto& f)
                                          {
                                              if (fanout_ntk.visited(f) == color_a)
                                              {
                                                  fanout_ntk.set_visited(f, color_a_and_b);
                                              }
                                              else
                                              {
                                                  fanout_ntk.set_visited(f, color_b);
                                              }
                                          });
            }

            std::vector<node> fanouts_a{};
            std::vector<node> fanouts_b{};
            ...
            // Collect exclusive fanouts of A (visited == color_a)
            for (const auto& x : frontier_a)
            {
                fanout_ntk.foreach_fanout(x,
                                          [&](const auto& f)
                                          {
                                              if (fanout_ntk.visited(f) == color_a)
                                              {
                                                  fanouts_a.push_back(f);

                                                  if (fanout_ntk.fanin_size(f) > 1)
                                                  {
                                                      node_a = backtrack_to_start_level(f, iter+2, color_a);
                                                      node_a_found = true;
                                                  }
                                              }
                                          });
            ...
            if (node_a != a && node_a_found)
            {
                return fanout_ntk.rank_position(node_a) < fanout_ntk.rank_position(root);
            }

            // Collect exclusive fanouts of B (visited == color_b)
            for (const auto& x : frontier_b)
            {
                fanout_ntk.foreach_fanout(x,
                                          [&](const auto& f)
                                          {
                                              if (fanout_ntk.visited(f) == color_b)
                                              {
                                                  fanouts_b.push_back(f);

                                                  if (fanout_ntk.fanin_size(f) > 1)
                                                  {
                                                      if (fanout_ntk.fanin_size(f) > 1)
                                                      {
                                                          node_b = backtrack_to_start_level(f, iter+2, color_b);
                                                          node_b_found = true;
                                                      }
                                                  }
                                              }
            ...
            if (node_b != b && node_b_found)
            {
                return fanout_ntk.rank_position(root) < fanout_ntk.rank_position(node_b);
            }

            if (fanouts_a.empty() && fanouts_b.empty())
            {
                // No unique explored nodes -> ordering is indifferent
                return true;
            }

            frontier_a = std::move(fanouts_a);
            frontier_b = std::move(fanouts_b);
        }

        // If never found a 2+-input node, fall back.
        return true;
    }
    ...
    {
        static_assert(mockturtle::has_is_buf_v<BufNtk>, "BufNtk does not implement the is_buf method");
        static_assert(mockturtle::has_create_buf_v<BufNtk>, "BufNtk does not implement the create_buf method");
        assert(!_outdated && "Please call `count_buffers()` first.");

        using buf_signal  = typename BufNtk::signal;
        using fanout_tree = std::vector<std::list<buf_signal>>;

        mockturtle::node_map<buf_signal, Ntk>  node_to_signal(_ntk);
        mockturtle::node_map<fanout_tree, Ntk> buffers(_ntk);
        ...
        /* PIs: Ensure same ranking and naming as in original network */
        _ntk.foreach_pi([&](auto const& n) { node_to_signal[n] = bufntk.create_pi(); });

        /* PIs buffer */
        _ntk.foreach_pi([&](auto const& n) { create_buffer_chain(bufntk, buffers, n, node_to_signal[n]); });
                ...
                _ntk.foreach_fanin(n,
                                   [&](auto const& fi)
                                   {
                                       ...
                                       if (_ntk.is_constant(_ntk.get_node(fi)))
                                           ...
                                           s = get_buffer_at_relative_depth(bufntk, buffers[fi],
                                                                            _levels[n] - _levels[fi] - 1);
        ...
        if (has_update_ranks_v<BufNtk>)
        {
            bufntk.update_ranks();
            // debug::write_dot_network(bufntk, "bufntk");
            if (_ps.planar)
            {
                // Here logic to reclaim planar rankings
                // Set PI ranks (redundant due to inserting logic)
                // bufntk.set_ranks(0, _ntk.get_ranks(0));
                for (auto lvl = 1u; lvl <= bufntk.depth(); lvl++)
                {
                    auto ntk_level = bufntk.get_ranks(lvl);
                    std::cout << "Level: " << lvl << '\n';

                    const auto barycenters = detail::compute_barycenters_level(bufntk, ntk_level);
                    ...
                    std::stable_sort(ntk_level.begin(), ntk_level.end(),
                                     [&](const auto& a, const auto& b)
                                     {
                                         ...
                                         if (a_bc == b_bc)
                                         {
                                             if (fanins(bufntk, a).fanin_nodes[0] != fanins(bufntk, b).fanin_nodes[0])
                                             {
                                                 int zz=0;
                                             }
                                             assert(fanins(bufntk, a).fanin_nodes[0] == fanins(bufntk, b).fanin_nodes[0]);
                                             return tie_break_by_fanout_walk(bufntk, a, b, fanins(bufntk, a).fanin_nodes[0]);
                                         }

                                         return a < b;
                                     });

                    bufntk.set_ranks(lvl, ntk_level);
                }
            }
        }
...
#pragma region Post-dump optimization
  public:
    template <typename BufNtk>
    void remove_buffer_and_splitter_chains(BufNtk& ntk) noexcept
    {
        using node   = typename BufNtk::node;
        using signal = typename BufNtk::signal;

        auto is_bypassable = [&](node const& n) -> bool
        {
            // Buffer nodes are bypassable if they are unary (one fanin).
            // Splitters are bypassable only if they are functionally a buffer (no logic).
            ...
            // These predicates must exist in your `technology_network`.
            // Replace `is_splitter` with your actual predicate / encoding.
            return ntk.is_buf(n) || ntk.is_fanout(n);
        };

        // Returns the unique fanin signal of a unary node.
        auto unary_fanin = [&](node const& n) -> signal
        {
            signal c{};
            bool   set = false;
        ...
        // For each node, chase through bypassable nodes to find the first non-bypassable driver.
        auto chase = [&](signal s) -> signal
        {
            while (true)
            {
                ...
                const auto in = unary_fanin(n);

                // If your signals can be complemented (e.g., AIG), preserve polarity:
                // s = ntk.is_complemented(s) ? (ntk.create_not(in)?) : in
                // In `klut_network` signals are plain indices (no complemented attr),
                // so this is typically just:
                s = in;
            }
        };

        // 1) Rewrite all gate fanins to bypass chains.
        ntk.foreach_gate(
            [&](node const& g)
            {
                // Collect current fanins
                std::vector<signal> fins;
                fins.reserve(ntk.fanin_size(g));
                ntk.foreach_fanin(g, [&](signal const& f) { fins.push_back(f); });

                bool changed = false;
                for (auto& f : fins)
                {
                    const auto f2 = chase(f);
                    if (f2 != f)
                    {
                        f       = f2;
                        changed = true;
                ...
                if (changed)
                {
                    // Rebuild gate with same function, new children.
                    // For KLUT-like networks: clone node truth table.
                    // You need a supported "replace node" primitive; mockturtle provides substitute_node.
                    const auto new_sig = ntk.clone_node(ntk, g, fins);
                    ntk.substitute_node(g, new_sig);
                }
            });

        // 2) Rewrite POs to bypass chains.
        ntk.foreach_po(
            [&](signal const& po, auto index)
            {
                const auto po2 = chase(po);
                if (po2 != po)
                {
                    // You need an API to set output at index; if not available, do manual storage update.
                    // For your `technology_network`, you already do `_storage->outputs[index] = ...` in
                    // `substitute_po_signals`.
                    ntk._storage->outputs[index] = po2;
                }
            });
    }
    ...
    uint32_t remove_buffer_chains(BufNtk& ntk) const
    {
        ...
        ntk.incr_trav_id();
        ntk.foreach_po([&](auto f) { remove_buffer_chains_rec(ntk, ntk.get_node(f), 0, max_chain); });
        return max_chain;
    }

  private:
    template <class BufNtk>
    std::pair<uint32_t, typename BufNtk::node> remove_buffer_chains_rec(BufNtk& ntk, typename BufNtk::node n,
                                                                        typename BufNtk::node parent,
                                                                        uint32_t&             max_chain) const
    {
        ...
        if (ntk.is_buf(n))
        {
            ...
            {
                ntk.foreach_fanin(n, [&](auto f) { remove_buffer_chains_rec(ntk, ntk.get_node(f), n, max_chain); });
                return std::make_pair(0, n);
            }
...
#pragma endregion
  ...
  public:
    ...
    void optimize()
    {
        if (_ps.optimization_effort == ranked_buffer_insertion_params::none)
        {
            return;
        }
        ...
        } while (updated && _ps.optimization_effort == ranked_buffer_insertion_params::until_sat);
...
#pragma region Chunked movement
  private:
    ...
    void recruit_fanouts(node const& n, chunk& c)
    {
        ...
        {
            for (auto it2 = it->extrefs.begin(); it2 != it->extrefs.end(); ++it2) c.po_interfaces.push_back({n, *it2});
        }
        ...
        if (_ps.assume.ci_capacity > 1 && _ntk.is_pi(n))
        {
            if (it->relative_depth == 1)
            {
                for (auto it2 = it->fanouts.begin(); it2 != it->fanouts.end(); ++it2) recruit(*it2, c);
                it++;
            }
    ...
    bool analyze_chunk_down(chunk c)
    {
        ...
        for (auto ii : c.input_interfaces)
        {
            ...
            if (_fanouts[ii.o].back().relative_depth == rd &&
                _fanouts[ii.o].back().num_edges == 0)  // `ii.c` is the last highest fanout of `ii.o`
            {
                ++c.benefits;
            }
        }
        ...
        if (c.benefits > 0 && c.slack > 0)
        {
            ...
            for (auto m : c.members) _levels[m] -= c.slack;
            if (!_ps.assume.balance_cios && c.slack >= _ps.assume.num_phases)
            {
                for (auto poi : c.po_interfaces) _po_levels[poi.o] -= c.slack;
            }
            for (auto m : c.members) update_fanout_info(m);
            for (auto ii : c.input_interfaces) legal &= update_fanout_info<true>(ii.o);
            ...
            if (!legal || num_buffers() >= buffers_before)
            {
                ...
                for (auto m : c.members) update_fanout_info(m);
                for (auto ii : c.input_interfaces) update_fanout_info(ii.o);
                _outdated = true;
                return false;
            }
    ...
    bool analyze_chunk_up(chunk c)
    {
        ...
        if (c.po_interfaces.size() > 0)
        {
            for (auto poi : c.po_interfaces)
            {
                if (_levels[poi.c] + num_splitter_levels(poi.c) + c.slack >= _po_levels[poi.o])
                {
                    if (_ps.assume.balance_cios)
                        c.slack = std::min(
                            c.slack, int32_t(_po_levels[poi.o] - _levels[poi.c] - num_splitter_levels(poi.c) - 1));
                    else
                    {
        ...
        if (c.benefits > 0 && c.slack > 0)
        {
            ...
            for (auto m : c.members) _levels[m] += c.slack;
            for (auto po : po_to_move) _po_levels[po] += c.slack;
            for (auto m : c.members) legal &= update_fanout_info<true>(m);
            for (auto ii : c.input_interfaces) legal &= update_fanout_info<true>(ii.o);
...
#pragma endregion
  ...
  private:
    ...
    Ntk const&                     _ntk;
    ranked_buffer_insertion_params _ps;
    bool                           _outdated{true};
    ...
    mockturtle::node_map<uint32_t, Ntk> _levels;
     ...
     */
    mockturtle::node_map<fanouts_by_level, Ntk> _fanouts;
    mockturtle::node_map<uint32_t, Ntk>         _num_buffers;

    mockturtle::node_map<std::pair<uint32_t, uint32_t>, Ntk> _timeframes;  // only for SMT; the most extreme min/max
    uint32_t                                                 _start_id;    // for chunked movement
}; /* buffer_insertion */

}  // namespace fiction
#endif  // FICTION_RANKED_BUFFER_INSERTION_HPP
