...
class crossing_reduction_impl
{
  public:
    ...
    mockturtle::rank_view<Ntk> run()
    {
        ...
        int num_sweeps = 1000;
        int64_t result_950, result_1000;
        for (int i = 0; i < num_sweeps; ++i)
        {
            ...
            for (uint32_t r = 0; r < rank_ntk_opt.depth(); ++r)
            {
                ...
                const auto [result, cost] =
                    simulated_annealing(rank_ntk_opt, ps.initial_temperature, ps.final_temperature, ps.number_of_cycles,
                                        crossing_cost, linear_temperature_schedule, next_rank_order);

                rank_ntk_opt = result;
                if (i == 949)
                {
                    result_950 = number_of_crossings(rank_ntk_opt);
                }
                else if (i == 999)
                {
                    result_1000 = number_of_crossings(rank_ntk_opt);
                }
            }
        }
        std::cout << "Improvement: " << (result_950 - result_1000) << std::endl;
