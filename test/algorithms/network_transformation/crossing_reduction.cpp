...
TEMPLATE_TEST_CASE("layer lattice straight-line crossing number", "[crossing-reduction]", fiction::technology_network)
{
    mockturtle::rank_view<TestType> rank_ntk{};

    // rank 0
    const auto cin = rank_ntk.create_pi();
    const auto y = rank_ntk.create_pi();
    const auto x = rank_ntk.create_pi();

    // rank 1
    const auto n7 = rank_ntk.create_and(y, x);
    const auto n10 = rank_ntk.create_and(x, y);
    const auto b11 = rank_ntk.create_buf(cin);
    const auto b13 = rank_ntk.create_buf(cin);

    // rank 2
    const auto n9 = rank_ntk.create_and(n7, b13);
    const auto n6 = rank_ntk.create_and(n7, b11);
    const auto b12 = rank_ntk.create_buf(n10);

    // rank 3
    const auto n8 = rank_ntk.create_and(n9, b12);
    const auto bx = rank_ntk.create_buf(n6);

    // outputs
    rank_ntk.create_po(bx);
    rank_ntk.create_po(n8);

    fiction::debug::write_dot_network(rank_ntk, "cross_ntk");
    ...
    const auto number_of_crossings_after_reduction = detail::number_of_crossings(reduced);
    std::cout << "\n" << "crossings before: " << number_of_crossings_before_reduction << std::endl;
    std::cout << "\n" << "crossings after: " << number_of_crossings_after_reduction << std::endl;

    /*rank_ntk.foreach_gate([&](const auto& fon) {
        std::cout << fon << std::endl;
    });*/
