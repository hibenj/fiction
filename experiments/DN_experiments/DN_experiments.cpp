...
#include "../../test/utils/blueprints/sequential_network_blueprints.hpp"
...
{
    auto name =  fmt::format("{}{}/{}.blif", EXPERIMENTS_PATH, benchmark_folder, benchmark_name);
    return name;
}
...
template <typename Ntk>
Ntk read_ntk_blif(const std::string& name)
{
    fmt::print("[i] processing {}\n", name);
    ...
    const auto read_blif_result =
        lorina::read_blif(benchmark_path_blif(name), mockturtle::blif_reader(network));
...
void ortho_sequential_mcs()
{
    ortho_stats = {};

    std::vector<std::string> const benchmark_names{"i99t/b01/b01"/*, "b02", "b03", "b04", "b05", "b06", "b07", "b08",
                                                   "b09", "b10", "b11", "b12", "b13", "b14", "b15", "b17"*/};
    ...
    for (const auto& benchmark : benchmark_names)
    {
        const auto ntk = read_ntk_blif<mockturtle::names_view<mockturtle::sequential<technology_network>>>(benchmark);

        // perform layout generation with an OGD-based algorithm
        auto lyt = fiction::orthogonal_sequential_network<gate_lyt>(ntk, {}, &ortho_stats);
...
void ortho_sequential_bpt_mcs()
{
    ...
    auto b01 = blueprints::seq_two<mockturtle::names_view<mockturtle::sequential<technology_network>>>();
    /*auto r2 = blueprints::maj_random_2<mockturtle::names_view<mockturtle::sequential<technology_network>>>();
    auto r3 = blueprints::maj_random_3<mockturtle::names_view<mockturtle::sequential<technology_network>>>();
    auto r4 = blueprints::maj_random_4<mockturtle::names_view<mockturtle::sequential<technology_network>>>();
    auto r5 = blueprints::maj_random_5<mockturtle::names_view<mockturtle::sequential<technology_network>>>();
    auto r6 = blueprints::maj_random_6<mockturtle::names_view<mockturtle::sequential<technology_network>>>();
    auto r7 = blueprints::maj_random_7<mockturtle::names_view<mockturtle::sequential<technology_network>>>();
    auto r8 = blueprints::maj_random_8<mockturtle::names_view<mockturtle::sequential<technology_network>>>();
    auto r9 = blueprints::maj_random_9<mockturtle::names_view<mockturtle::sequential<technology_network>>>();
    auto r10 = blueprints::maj_random_10<mockturtle::names_view<mockturtle::sequential<technology_network>>>();
    auto r11 = blueprints::maj_random_11<mockturtle::names_view<mockturtle::sequential<technology_network>>>();*/

    std::vector<mockturtle::names_view<mockturtle::sequential<technology_network>>> bms_tech = {b01};

    std::vector<std::string> const benchmark_names {"b01"};
