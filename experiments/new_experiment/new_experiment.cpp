...
int main()  // NOLINT
{
    ...
    static constexpr uint64_t bench_select = fiction_experiments::fontes18;  // xor5_maj
    ...
    static constexpr benchmark_source selected_benchmark_source = benchmark_source::fiction_only;
    ...
    for (const auto& benchmark : benchmarks_to_run)
    {
        ...
        constexpr uint64_t max_pr_nodes = 25000ull;
        ...
        // Toggle human-readable DRV reports emitted by gate_level_drvs.
        bool const print_drv_report = true;
        ...
        if (use_plane_pr)
        {
            ...
            else
            {
                try
                {
                    ...
                    std::ostringstream             suppressed_drv_output{};
                    drv_ps.out = print_drv_report ? &std::cout : &suppressed_drv_output;

                    if (print_drv_report)
                    {
                        fmt::print("\n[i] DRV report (new) for {}\n", benchmark);
                    }
                    fiction::gate_level_drvs(layout_plane_new, drv_ps, &stats);
            ...
            else
            {
                try
                {
                    ...
                    if (print_drv_report)
                    {
                        fmt::print("\n[i] DRV report (old) for {}\n", benchmark);
                    }
                    fiction::gate_level_drvs(layout_plane_old, drv_ps, &stats);
